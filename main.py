...
# Initialize logger
logging.basicConfig(filename=f'logs/{datetime.now().strftime("%Y-%m-%d-%H-%M-%S")}.log', encoding='utf-8', level=logging.INFO)
...
def save_image(imageData: dict, imageNumber: str):
    try:
        ...
        urllib.request.urlretrieve(imageData['link'], f'images/{imageData["query"]}_{imageNumber}.jpg')
...
if __name__ == "__main__":  
    ...
    with Pool(8) as p:
        ...
        try:
            ...
            imageMetaData = pd.concat([currentCSV, pd.DataFrame(allImageData)])
            imageMetaData.reset_index(drop=True, inplace=True)
        except:
            imageMetaData = pd.DataFrame(allImageData)

        print("Saving dataframe to CSV")
        imageMetaData.to_csv('data.csv')
